...
wincc_error_t* wincc_connect(wincc_client_t* client) {
    ...
    char* variables = cJSON_Print(variables_obj);
...
wincc_tag_results_t* wincc_read_tags(wincc_client_t* client, const char** tag_names, size_t count) {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        cJSON* name_item = cJSON_CreateString(tag_names[i]);
...
wincc_write_results_t* wincc_write_tags(wincc_client_t* client, const wincc_tag_write_t* tags, size_t count) {
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        cJSON_AddStringToObject(tag_obj, "value", tags[i].value);
...
wincc_browse_results_t* wincc_browse(wincc_client_t* client, const char* path) {
    ...
    if (path) {
        cJSON* path_item = cJSON_CreateString(path);
        cJSON_AddItemToArray(name_filters, path_item);
    }
    
    cJSON_AddItemToObject(variables_obj, "nameFilters", name_filters);
    char* variables = cJSON_Print(variables_obj);
    cJSON_Delete(variables_obj);
...
wincc_error_t* wincc_acknowledge_alarm(wincc_client_t* client, const char* alarm_id) {
    ...
    if (*endptr == '\0') {
        ...
        cJSON_AddStringToObject(alarm_obj, "name", "");
        cJSON_AddNumberToObject(alarm_obj, "instanceID", instance_id);
    } else {
        // It's a name
        cJSON_AddStringToObject(alarm_obj, "name", alarm_id);
        cJSON_AddNumberToObject(alarm_obj, "instanceID", 0);
    }
    
    cJSON_AddItemToArray(input_array, alarm_obj);
    cJSON_AddItemToObject(variables_obj, "input", input_array);
